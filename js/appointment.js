        ...
        async function fetchSpecialists() {
            try {
                ...
                console.log('Backend URL: `${BASE_URL}/api/admin/specialists`');

                const response = await fetch(`${BASE_URL}/api/admin/specialists`, {
                    method: 'GET',
                    headers: {
        ...
        function showFallbackSpecialists() {
    specialists = [
        {
            id: 1,
            name: "Priya Sharma",
            ...
            bio: "Helping people manage stress and anxiety effectively.",
            experience: 5,
            rating: 4.8,
            reviews: 120,
            avatar: "P",
            languages: "English, Hindi",
            availability: generateWeeklyAvailability(1)
        },
        {
            id: 2,
            name: "Rahul Verma",
            ...
            bio: "Expert in relationship and communication issues.",
            experience: 7,
            rating: 4.7,
            reviews: 98,
            avatar: "R",
            languages: "English, Hindi",
            availability: generateWeeklyAvailability(2)
        }
    ];
    ...
    populateSpecialists();
        ...
        function setupEventListeners() {
            ...
            // });
                ...
                document.getElementById('timeSlots').addEventListener('click', function(e) {
                const slot = e.target.closest('.time-slot-card');

                if (slot && slot.classList.contains('available')) {
                    document.querySelectorAll('.time-slot-card')
        ...
        function updateTimeSlots() {
            ...
            timeSlotsContainer.innerHTML = daySlots.map((slot, index) => {
                ...
                // `;
                return `
                    <div class="time-slot-card ${classes}" data-time="${slot.time}" 
                        style="animation-delay: ${index * 0.1}s;">
        ...
        async function fetchAppointments() {
            try {
                ...
                const response = await fetch(`${BASE_URL}/api/appointments/all`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                    },
        ...
        function viewDirections() {
            const address = "CalmSpace Center, Main Branch";
