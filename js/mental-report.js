document.addEventListener('DOMContentLoaded', function() {
    ...
    async function generateAndDownloadPDF(reportData) {
        ...
        function generatePDFContent() {
            ...
            yPosition += lineHeight;
            const disclaimerText = 'This report is generated by CalmSpace AI system and is for informational purposes only. It should not be considered as a substitute for professional medical advice, diagnosis, or treatment. Please consult with a qualified healthcare professional for proper evaluation and treatment.';
            const disclaimerHeight = addWrappedText(disclaimerText, 20, yPosition, 170);
            yPosition += disclaimerHeight + 10;

            doc.text(`© ${new Date().getFullYear()} CalmSpace. All rights reserved.`, 20, yPosition);
            doc.text(`Generated on: ${new Date().toLocaleString()}`, 120, yPosition);

            // Save the PDF
            const fileName = `CalmSpace-Mental-Health-Report-${reportDate.replace(/[/:,]/g, '-')}.pdf`;
            doc.save(fileName);
        }
    }
