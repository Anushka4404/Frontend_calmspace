    ...
    async function analyzeMoodImage(formData) {
        try {
            ...
            if (data && data.mood !== undefined && data.moodLabel) {
                ...
                if (moodLabel === "Angry") {
                    setTimeout(() => {
                        window.location.href = "bubble-game.html";
                    }, 2000); // 2 sec delay so user can see result
                }
                else if (moodLabel === "Neutral" || moodLabel === "Sad") {
                    window.location.href = "gratitude-game.html";
                }
